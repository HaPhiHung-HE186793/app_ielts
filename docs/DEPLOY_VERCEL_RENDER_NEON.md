...
- Nếu hostname Connect khác cấu hình Render trước đó, kiểm tra project/branch/compute đang chọn trước khi sửa env. Ảnh mới có endpoint ep-dawn-dream, trước là ep-long-cell; chưa có xác nhận đổi DB. Tên project không tạo PostgreSQL role. Dùng SQL Editor để kiểm tra catalog; dropdown một lựa chọn chưa đủ kết luận migration thiếu. DATABASE_URL và Auth Base URL phải được lấy từ project/branch dự định dùng, không tự ghép hai ảnh khác endpoint. [Neon SQL Editor](https://neon.com/docs/get-started/query-with-neon-sql-editor).
