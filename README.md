...
- **Vercel + Render + Neon:** DATA-003 đã có adapter Auth/DB/API, migration riêng và cấu hình build. Đã kiểm tra PostgreSQL local và luồng tài khoản/đồng bộ với Auth REST mô phỏng; chưa kiểm tra dịch vụ Neon thật. Người dùng có Neon (`production`/`neondb`, Singapore) và đang ở form Render/Vercel. **Tiếp theo DEPLOY-002:** áp migration, bật Neon Auth, nhập env và deploy theo [hướng dẫn từng trường](docs/DEPLOY_VERCEL_RENDER_NEON.md). Chưa có URL public đã xác minh. Kiến trúc/giới hạn tại [NEON_BACKEND.md](docs/NEON_BACKEND.md); Supabase local giữ cho regression, AI vẫn tắt.
