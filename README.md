...
- **Vercel + Render + Neon:** DATA-003 có adapter Auth/DB/API, migration và kiểm tra PostgreSQL local/Auth REST mô phỏng. Render `app_ielts` đã build thành công, startup đang dừng do DATABASE_URL dùng owner thay runtime role. **Tiếp theo DEPLOY-002:** lấy kết nối `moi_ngay_runtime`, sửa env/redeploy Render rồi tiếp tục Vercel theo [hướng dẫn](docs/DEPLOY_VERCEL_RENDER_NEON.md). Auth Base URL đã có ở STATUS, nhưng DB/Auth/cloud/URL public chưa xác minh. Kiến trúc/giới hạn tại [NEON_BACKEND.md](docs/NEON_BACKEND.md); Supabase local giữ cho regression, AI vẫn tắt.
