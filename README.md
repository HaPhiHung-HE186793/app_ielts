...
- **Vercel + Render + Neon:** DATA-003 có adapter/migration và kiểm tra local. Render `app_ielts` build đạt, đã chọn runtime role và qua validator, nhưng kiểm tra DB/schema/quyền chưa thành công. Đã thêm log `NEON_DB_*` và [SQL kiểm tra](db/neon/check_setup.sql). **Tiếp theo DEPLOY-002:** deploy bản mới, đối chiếu log/query để sửa đúng lỗi rồi tiếp tục Vercel theo [hướng dẫn](docs/DEPLOY_VERCEL_RENDER_NEON.md). DB/Auth/cloud/URL public chưa xác minh; cần đối chiếu Auth URL cũ với project đang chọn. Supabase local giữ cho regression, AI vẫn tắt.
