...
2. Chọn **Tải gói để học offline**. Gói “Bốn tuần nền tảng” chứa 57 WAV và JSON của 28 bài + bốn bài kiểm tra. Chờ **Sẵn sàng học offline**; có số file/dung lượng và nút kiểm tra.
...
6. **Xóa gói tải xuống** xóa file học/audio đã tải, giữ phần mở app, bài dở, lịch ôn và outbox. Nội dung bài cũng nằm trong mã app nên phần đọc vẫn có thể mở; phần nghe cần mạng hoặc tải lại. Đây không phải nút xóa tiến độ/tài khoản.
...
- 57 file ở `public/packs/foundation-v1/` được tạo từ đúng `lesson.phrase` và `exercise.audioText` bằng **eSpeak NG 1.51**, giọng formant `en-us`, tốc độ 145 từ/phút. Chỉ phân phối đầu ra câu nói, không đưa chương trình hay bộ dữ liệu giọng vào app. Không có bản thu người học/giáo viên hoặc giọng người được mô phỏng.
...
- Phần mở app có cache `moi-ngay.shell.<hash-build>`, tự tải lúc worker cài lần đầu; khoảng 1,15 MB chưa nén. Gói 8.921.491 byte, khoảng 8,92 MB, tên `moi-ngay.pack.<version-hash>`, chỉ tải khi người học chọn. UI báo kích thước byte của tài nguyên (MB = 1.000.000 byte), không coi đó là quota còn trống; trình duyệt có overhead và nén khác nhau.
...
- Giữ StudyState version 3 và kho tiến độ hiện có. Cần cập nhật app ở máy khác trước khi nhập/sync bài mới; bản cũ chưa biết các ID mới. Cache Storage phù hợp file HTTP công khai; chưa cần chuyển tiến độ sang IndexedDB. Lỗi quota tiến độ và bảo vệ bản lỗi của DATA-002 vẫn áp dụng.

CONTENT-002 mở rộng từ 8 lên 58 tài nguyên. Worker vẫn kiểm tra từng file, cập nhật tiến độ sau mỗi nhóm tám file và ở cuối để giảm số lượt quét cache; kết quả cuối vẫn quét đủ trước báo sẵn sàng. Script audio giữ file có cùng câu/hash, chỉ tạo phần mới/đổi trong thư mục local riêng; lần mở rộng giữ bảy WAV cũ nguyên byte. Bài nghe tình huống có nút xem lời thoại như gợi ý, giữ qua reload; xem [CURRICULUM.md](CURRICULUM.md).
