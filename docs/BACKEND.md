...
DATA-001 đã có Supabase Auth thật trên Docker local, xác nhận email bằng mã và bảng tên tài khoản có RLS. Chưa có dự án Supabase cloud, SMTP gửi thư thật hoặc đồng bộ bài học. Xem [DEC-013](DECISIONS.md#dec-013--tài-khoản-bằng-mã-email-và-kho-học-theo-chủ-sở-hữu).
...
4. Đăng xuất để trở lại phần khách. Đăng nhập lại cùng tài khoản trên cùng origin để mở phần học đã giữ trên máy.
...
CLI quản lý migration trong database; `db:start` là đường khởi tạo stack mới. Nếu thêm migration khi stack đã tồn tại, áp dụng migration local theo quy trình CLI và kiểm tra trước khi chuyển sang cloud. Không chạy `db reset` trên dữ liệu muốn giữ.
...
| Tên gọi học tập, mục tiêu, câu trả lời, bài dở, lịch ôn, thời gian | localStorage riêng theo origin ứng dụng, URL backend và ID tài khoản |
| Phần học không đăng nhập | Key cũ `moi-ngay.study.v1`, giữ nguyên dữ liệu version 1/2/3 |

Đăng xuất xóa phiên SDK trên trình duyệt và ẩn phần học của tài khoản, không xóa bài đang lưu. Kho học chưa mã hóa; đổi tài khoản chỉ tách giao diện, không bảo vệ khỏi người có quyền đọc dữ liệu trình duyệt/máy. Khi dùng máy chung, tải bản sao rồi xóa phần học hiện tại trong cài đặt trước khi đăng xuất. Nhập/xóa bản sao chỉ tác động phần học đang mở, không xóa Auth user hoặc tên trên server.
...
Đổi chủ sở hữu tăng epoch trước khi đổi kho và dựng lại form, ngăn bộ đo hoặc thao tác đọc file cũ ghi vào phần học mới. Auth cập nhật qua các tab trong cùng origin. Chưa hợp nhất an toàn các thay đổi học đồng thời trên nhiều tab; đồng bộ, hàng đợi và nhập dữ liệu khách có lựa chọn thuộc DATA-002.
